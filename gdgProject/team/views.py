...
def _build_suggested_members(team):
    ...
    return sorted(
        candidates,
        key=lambda candidate: (-candidate["score"], candidate["display_name"].lower()),
    )[:5]
...
def create_team(request, event_id):
    ...
    try:
        ...
        logger.info(
            "Team '%s' created for event %d by user %d", name, event.pk, request.user.pk
        )
        ...
        messages.error(
            request,
            "A team with that name already exists for this event, or you already lead a team.",
        )
        return redirect("events:event_detail", event_id=event.pk)
...
def leave_team(request, team_id):
    ...
    membership.delete()
    logger.info(
        "User %d left team %d for event %d", request.user.pk, team.pk, team.event.pk
    )
...
def remove_member(request, team_id, user_id):
    ...
    membership.delete()
    logger.info(
        "User %d removed from team %d by leader %d", user_id, team.pk, request.user.pk
    )

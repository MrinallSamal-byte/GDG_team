...
from registration.models import Registration, RegistrationStatus, RegistrationTechStack, RegistrationType
...
class TeamManagementViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.candidate = User.objects.create_user(
            username='candidate', password='testpass123', first_name='Cand', last_name='Idate',
        )
    ...
    def test_chat_post_valid_message(self):
        self.client.login(username='teamtest', password='testpass123')
        resp = self.client.post(self.url, {'action': 'send_message', 'message': 'Hello team!'})
        self.assertRedirects(resp, self.url)
    ...
    def test_team_leader_sees_suggested_members(self):
        ...
        RegistrationTechStack.objects.create(registration=registration, tech_name='React', is_primary=True)

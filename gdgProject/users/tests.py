...
class LoginViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            ...
            password='testpass123', first_name='Login',
        )
    ...
    def test_login_success(self):
        resp = self.client.post(self.url, {'email': 'login@example.com', 'password': 'testpass123'})
    ...
    def test_login_nonexistent_email(self):
        resp = self.client.post(self.url, {'email': 'no@example.com', 'password': 'pass'})
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, 'Invalid email or password')
    ...
    def test_login_repopulates_email(self):
        resp = self.client.post(self.url, {'email': 'test@example.com', 'password': 'wrong'})
...
class RegisterViewTest(TestCase):
    ...
    def test_register_missing_fields(self):
        resp = self.client.post(self.url, {'full_name': '', 'email': '', 'password': ''})
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, 'Please fill in all required fields')
...
class EmailVerificationViewTest(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(
            username='verifytest', password='testpass123',
        )
    ...
    def test_valid_otp(self):
        resp = self.client.post(self.url, {
            'otp_1': '1', 'otp_2': '2', 'otp_3': '3',
            'otp_4': '4', 'otp_5': '5', 'otp_6': '6',
        })
        self.assertRedirects(resp, reverse('dashboard:user_dashboard'))

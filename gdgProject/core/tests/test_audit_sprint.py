    ...
    def test_unique_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            RegistrationTechStack.objects.create(
                registration=self.reg, tech_name="Flask"
            )
...
class TestTeamManagementViews(TestCase):
    ...
    def setUp(self):
        ...
        self.outsider = User.objects.create_user(
            username="outsider", password="pass1234!!"
        )
        self.event = Event.objects.create(
            title="Team Event",
            slug="team-evt-mgmt",
        ...
        TeamMembership.objects.create(
            team=self.team, user=self.leader, role=MemberRole.BACKEND
        )
        TeamMembership.objects.create(
            team=self.team, user=self.member, role=MemberRole.FRONTEND
        )
    ...
    def test_leader_cannot_leave_team(self):
        ...
        self.assertEqual(resp.status_code, 302)
        self.assertTrue(
            TeamMembership.objects.filter(team=self.team, user=self.leader).exists()
        )
...
class TestPasswordReset(TestCase):
    ...
    def test_forgot_password_post_with_valid_email(self):
        resp = self.client.post(
            "/auth/forgot-password/", {"email": "reset@example.com"}
        )
        self.assertEqual(resp.status_code, 302)
...
class TestNotificationCreation(TestCase):
    ...
    def setUp(self):
        ...
        self.leader = User.objects.create_user(
            username="notif_leader", password="pass1234!!"
        )
        self.requester = User.objects.create_user(
            username="notif_req", password="pass1234!!"
        )
        self.event = Event.objects.create(
            title="Notif Event",
            slug="notif-evt",
    ...
    def test_join_request_creates_notification_for_leader(self):
        ...
        notif = Notification.objects.filter(
            user=self.leader, type="join_request"
        ).first()
    ...
    def test_approve_creates_notification_for_requester(self):
        ...
        svc.approve_request(
            team_id=self.team.pk,
            requester_user_id=self.requester.pk,
            approver=self.leader,
        )
        notif = Notification.objects.filter(
            user=self.requester, type="request_approved"
        ).first()
        self.assertIsNotNone(notif)
    ...
    def test_decline_creates_notification_for_requester(self):
        ...
        svc.decline_request(
            team_id=self.team.pk,
            requester_user_id=self.requester.pk,
            decliner=self.leader,
        )
        notif = Notification.objects.filter(
            user=self.requester, type="request_declined"
        ).first()
        self.assertIsNotNone(notif)

...
def _normalize_member_role(selected_role):
    ...
    if not selected_role:
        return 'other'
    return _ROLE_VALUE_MAP.get(selected_role, selected_role if selected_role in _ROLE_VALUE_MAP.values() else 'other')
...
def _send_confirmation_email(registration):
    ...
    try:
        ...
        logger.error(
            ...
            registration.registration_id, exc, exc_info=True,
        )
...
def register_event(request, event_id):
    ...
    if event.participation_type in ('team', 'both'):
        ...
        open_teams = event.teams.filter(
            status='open', is_deleted=False,
        ).annotate(current_members=Count('memberships')).select_related('leader')
    ...
    if request.method == 'POST':
        ...
        if reg_type == RegistrationType.INDIVIDUAL:
            ...
            logger.info(
                ...
                registration.registration_id, event.pk, request.user.pk,
            )
            messages.success(
                request,
        ...
        if reg_type == RegistrationType.TEAM and team_action == 'create':
            if not team_name:
                ...
                return render(request, 'registration/register_event.html', {
                    'event': event, 'roles': _ROLES, 'skills': _SKILLS,
                    'form_data': request.POST, 'custom_fields': custom_fields,
                    'open_teams': open_teams,
                })
            ...
            logger.info(
                ...
                team_name, registration.registration_id, event.pk, request.user.pk,
            )
            messages.success(
                request,
